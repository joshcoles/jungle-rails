  ...
  describe 'Validations' do
   ...
   before :each do
    ...
    it 'should be invalid without a price' do
      @product = Product.new(name: 'Josh', price: nil, quantity: 2, category: @category)
      expect(@product).to_not be_valid
      expect(@product.errors[:price]).to include("can't be blank")
    end

    it 'should be invalid without a quantity' do
      @product = Product.new(name: 'Josh', price: 20, quantity: nil, category: @category)
      expect(@product).to_not be_valid
      expect(@product.errors[:quantity]).to include("can't be blank")
    end

    it 'should be invalid without a price' do
      @product = Product.new(name: 'Josh', price: 20, quantity: 2, category: nil)
      expect(@product).to_not be_valid
